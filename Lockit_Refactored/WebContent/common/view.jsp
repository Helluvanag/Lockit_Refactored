<%@ page import=" com.verudix.lockit.flexpaperviewer.*,java.io.*" %>
<%
	String logged_user = (String)session.getAttribute("user_name");
	String logged_user_mail = (String)session.getAttribute("user_mail");
	String pdfPath = getServletContext().getRealPath("/")+"Userfiles"+File.separator+logged_user_mail+File.separator+"convertedPdfs"+File.separator;
	String swfPath = getServletContext().getRealPath("/")+"Userfiles"+File.separator+logged_user_mail+File.separator+"convertedSwfs"+File.separator;
	System.out.println("The pdfPath is---"+pdfPath);
	System.out.println("The swfPath is---"+swfPath);
	...
	String messages 	= "";
	String swfFilePath 	= swfPath + swfdoc;
	String pdfFilePath 	= pdfPath + pdfdoc;
	String pngFilePath 	= swfPath + pngdoc;
	String jpgCachePath 	= swfPath + jpgcachedoc;
	String jsonFilePath 	= swfPath + jsondoc;
	...
	if(!validatedConfig){
		...
		if("swf".equals(format) || "jpg".equals(format) || "png".equals(format) || "pdf".equals(format)){
			if(!conf.file_exists(swfFilePath)){
				pdf2swf pdfconv = new pdf2swf(request, pdfPath, swfPath);
			...
			if(!conf.file_exists(jsonFilePath)){
				pdf2json jsonconv = new pdf2json(pdfPath, swfPath);
